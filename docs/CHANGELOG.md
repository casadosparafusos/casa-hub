...
FASE E (`feat/measured-packages-admin`, base atual `main` `4a59960d` já com FASE C/C.1/C.2 (PR #3) e FASE D-PRE (PR #4) mergeadas): CRUD + importação por planilha de Embalagens KG/MT, sobre o motor matemático e a tabela `product_sale_unit_config` já existentes desde a FASE B/B.2 (não duplicados/reprojetados). UNIT continua vindo só do CISS; a planilha só fornece `quantity_per_sale_unit`. Escopo só código local + migration nova de histórico/auditoria + APIs internas autenticadas + UI `/embalagens` + testes, sem produção/SSH/deploy/migration em produção/escrita real Wake/CISS/merge desta fase. **PR #5 aberto como Draft** — primeira revisão do Tech Lead: `REQUEST_CHANGES` (correções em andamento na mesma branch/PR). Ver detalhes em `STATUS.md` (seção "FASE E").
...
- **PR #5 aberto como Draft, `REQUEST_CHANGES` na primeira revisão do Tech Lead — correções em andamento na mesma branch/PR.** Não mergeado, não deployado, nenhuma migration aplicada em produção, nenhuma escrita real em Wake/CISS, os 16 PC não corrigidos, KG real não remediado, promoção 10365 inalterada, FASE D real de rollout não iniciada.

## v13 — 18/09/2026

FASE D-PRE (`fix/preprod-static-hardening`, sobre `main` já com `feat/write-guards-readback` mergeada via PR #3, `5d94b9b`): hardening estático pré-produção a partir de achados de auditoria independente do Tech Lead direto sobre o repositório público — FASE D.0 (rollout real) ficou BLOCKED por SSH/credenciais, então a revisão passou a ser estática. Escopo só código+testes+documentação, sem merge/deploy/migration em produção/escrita real em Wake/CISS. **Mergeada em `main` via PR #4 (`4a59960d`).** Ver detalhes em `STATUS.md` (seção "FASE D-PRE").

Mudanças (primeira rodada de revisão do Tech Lead):
- **Tabela 74 — releitura pós-escrita direcionada por item, sem segundo full-scan**: `syncPrices()` (`src/lib/sync/engine.ts`) relia a tabela inteira DENTRO do loop de cada lote (P páginas * B lotes de GETs, risco real de estourar o rate limit da Wake); passou primeiro (nesta rodada) a escrever todos os lotes antes e fazer uma única releitura paginada final; uma segunda rodada de revisão do mesmo Tech Lead (mesmo PR #4, ver bullet dedicado abaixo) substituiu essa releitura final por uma leitura direcionada por item, sem `try/catch` faltante e sem depender do tamanho da tabela;
...
- 514 testes no total (443 da FASE C.2 + 71 líquidos novos) ao final desta primeira rodada.

Mudanças (segunda rodada de revisão do Tech Lead, mesmo PR #4, mesmo dia — 5 achados adicionais):
- **Lock token único por aquisição (fix #1)**: `ownerTag()` (`src/lib/sync/lock.ts`) passou a incluir um nonce via `randomUUID()` a cada `acquireLock()` — mecanismo NOVO desta rodada (a nota anterior deste changelog dizia incorretamente que já existia de uma fase anterior); `releaseLock()` só limpa se o token bater com o dono atual. `src/lib/sync/lock.test.ts`: 13 testes;
- **Releitura final da Tabela 74 em `try/catch`, nunca mais `'mismatch'` por falha de leitura (fix #2)**: a releitura paginada final (bullet acima) não estava protegida — uma falha de rede na própria verificação podia derrubar a run inteira depois de todos os lotes já escritos;
- **`WAKE_STOCK_CONTROL_MODE` valida `fstore`|`erp`; `CSV_IDENTIFIER_TYPE` removida (fix #3)**: nova validação de enum (`SETTING_ENUM_RANGES`/`validateEnumSettingValue`) rejeita qualquer valor fora de `fstore`/`erp` (case-insensitive, persiste canônico em minúsculo) tanto no bloqueio de sync real quanto no `PUT /api/settings`; `CSV_IDENTIFIER_TYPE` removida de `REQUIRED_UNCONFIRMED_KEYS` (5 → 4 chaves) após auditoria confirmar zero consumidor real (`tipoIdentificador` é sempre literal hardcoded em `src/lib/wake/client.ts`);
- **Tabela 74 — leitura direcionada por item substitui o segundo full-scan (fix #4)**: `readWakePriceTableByVariantId(variantId, tableId)` novo (`src/lib/wake/client.ts`) reconfere cada item escrito individualmente, serializado com `WAKE_VERIFY_DELAY_MS`, em vez de uma releitura paginada da tabela inteira — fórmula de requests passa a ser 1 leitura paginada inicial (diff) + N leituras direcionadas (1 por item escrito), nunca mais um segundo full-scan;
- **Espaços em branco nas settings numéricas (fix #5)**: `validateSettingValue()` agora faz `raw.trim()` antes de `Number()` (só-espaços rejeitado, nunca `0`/`NaN` silencioso); achado um bug relacionado ao aplicar a correção — `PUT /api/settings` validava o valor trimado mas persistia o valor original não-trimado; corrigido junto (persiste sempre `value.trim()` para numéricas, valor canônico para enum);
- 542 testes no total (514 da primeira rodada + 28 líquidos novos); `tsc --noEmit`, `vitest run` (24 arquivos) e `npm run build` limpos; scan de segredos/`.env`/`artifacts-private/`/dado-por-SKU limpo;
- **não mergeado nesta rodada (mergeada posteriormente em `main` via PR #4, `4a59960d`)**, não deployado, nenhuma migration aplicada em produção, nenhuma escrita real em Wake/CISS, os 16 PC não corrigidos, KG não remediado, promoção 10365 inalterada, FASE D real de rollout continua BLOCKED por SSH/credenciais.

Mudanças (revisão final #2 / terceira rodada de revisão do Tech Lead, mesmo PR #4, mesmo dia — 6 achados adicionais + fix de gates):
- **`WAKE_STOCK_CONTROL_MODE='erp'` passa a bloquear escrita real de estoque (fix #1)**: `erp` era um valor válido (fix #3 da rodada anterior validava o enum, mas não impedia uso real), e liberava um fluxo de escrita real de estoque que o app não implementa (nenhum write-off por pedido). `runSyncLocked()` (`src/lib/sync/engine.ts`) agora lança `WAKE_STOCK_CONTROL_MODE_UNSUPPORTED_FOR_REAL_STOCK_SYNC` ANTES de criar o `sync_run` e antes de qualquer writer, sempre que `kind` inclui estoque (`'stock'`/`'both'`) E `dryRun===false` E o modo é `erp`; `dryRun:true` e `kind:'price'` nunca são afetados. Nenhuma implementação de baixa de estoque por pedido foi feita nesta rodada — fora de escopo, ver `ARCHITECTURE_TARGET.md`;
- **Tabela 74 — UPDATE e ADD com `try/catch` independentes (fix #2)**: dentro de cada lote de `syncPrices()`, o sub-lote de atualização (`updateBatch`) e o de inclusão (`addBatch`) agora falham de forma isolada — antes uma falha em qualquer um dos dois HTTP calls do lote derrubava o lote inteiro (inclusive o outro sub-lote que teria sucesso). 3 testes novos cobrindo UPDATE falha/ADD sucede, UPDATE sucede/ADD falha, e ambos sucedem (misto real, 4 itens, 2 UPDATE + 2 ADD);
- **`readWakePriceTableByVariantId()` ganha teste direto (fix #3)**: cobertura unitária isolada da função de releitura direcionada por item introduzida na segunda rodada (fix #4), que antes só era exercitada indiretamente via `syncPrices()`;
- **Teste genuíno multi-lote (3 lotes / 110 produtos) (fix #4)**: teste novo que efetivamente excede `WAKE_BATCH_SIZE=50` em mais de um lote (110 produtos → 3 lotes reais), substituindo um teste anterior que simulava a condição sem de fato gerar múltiplos lotes;
- **Comentário desatualizado em `engine.ts` corrigido (fix #5)**: referência a "120 req/min" no comentário sobre o rate limit da Wake removida — o valor não reflete nenhuma constante ou limite real verificado no código, só a explicação do bloqueio de 1h após 5x 429 seguidos permanece;
- **`CSV_IDENTIFIER_TYPE` removida do `.env.example` (fix #6)**: variável morta (zero consumidor real, `tipoIdentificador` é sempre literal hardcoded em `src/lib/wake/client.ts`) já havia sido removida de `REQUIRED_UNCONFIRMED_KEYS` na rodada anterior (fix #3); esta rodada limpa a referência residual no arquivo de exemplo de env;
- **Correção de precisão em `STATUS.md`/`ARCHITECTURE_TARGET.md` (item 7)**: a nota da rodada anterior sobre `WAKE_STOCK_CONTROL_MODE` afirmava que um valor inválido seria "silenciosamente tratado como `erp`" pelo motor de sync — impreciso, não existe consumidor runtime que ramifique nessa setting dentro de `runSyncLocked()`/`syncPrices()`/`syncStock()`; a setting é um gate/registro operacional, validado na gravação e no pré-flight, nunca no motor de sync. Corrigido nos dois arquivos, com referência cruzada pro fix #1 desta rodada (que fecha a lacuna real: `erp` válido mas sem suporte a escrita real de estoque);
- **3 bugs reais encontrados ao rodar os gates completos pela primeira vez nesta rodada**: 2 bugs em testes novos desta própria rodada (`engine.test.ts` — timeout por falta de fake timers num teste com 4 releituras seriadas da Tabela 74; status `'partial'` inesperado por mock de preço unitário não isolado do foco do teste; teste de guard `dryRun` com vazamento de estado entre duas chamadas de `runSync()` na mesma DB não resetada) e 1 erro de `tsc --noEmit` (import de tipo não utilizado, `WakePriceTableProductItem`, sobrando da refatoração do fix #2). Todos corrigidos e reverificados;
- 560 testes no total (542 da rodada anterior + 18 líquidos novos); `tsc --noEmit`, `vitest run` (24 arquivos) e `npm run build` limpos; scan de segredos/`.env`/`artifacts-private/`/dado-por-SKU limpo;
- **não mergeado nesta rodada (mergeada posteriormente em `main` via PR #4, `4a59960d`)**, não deployado, nenhuma migration aplicada em produção, nenhuma escrita real em Wake/CISS, nenhum PR novo, nenhum force-push — FASE D real de rollout continua BLOCKED por SSH/credenciais.
