...
Atualizado em 16/09/2026 (FASE B.2: final hardening dos `UnitStrategy` em `feat/unit-strategies`, fecha os 5 bloqueios A-E apontados na revisão da FASE B.1 — aguardando revisão e autorização do usuário — não mergeado, não deployado, nenhuma migration aplicada em produção).
...
| `feat/unit-strategies` | `main` (`4ebedec`) | **Branch atual (FASE B + FASE B.1 + FASE B.2).** Implementa o motor `UnitResolver → UnitStrategy → CommercialPolicy` (mapa OWNER_CONFIRMED), integra `sync/engine.ts` (`syncPrices()`/`syncStock()`) ao motor, persiste a UNIT observada por produto, cria a tabela `product_sale_unit_config`. FASE B.1 (hardening) somou `WHOLESALE_DISCOUNT_PERCENT`/`getCommercialPolicyConfig()` (conecta `FIXADOR_CENTO` aos settings em vez de hardcoded), isolamento writer-spy nos testes de sync, testes de integridade de `product_sale_unit_config` contra banco real, testes de fronteira de precisão monetária e prova de fail-closed pra UNIT sem registro. FASE B.2 (final hardening) fechou os 5 bloqueios apontados na revisão: CHECK `source_unit IN ('KG','MT')` no banco (migration `0004_public_betty_brant.sql`), remoção de `wake_sku` denormalizado da tabela, prova de escrita real isolada por UNIT (DIRECT/HUNDRED/PACKAGE_MEASURED), `moneyRound` reescrito (bug real corrigido: `10.075` virava `10.07`) e status distinto `NO_STOCK_RECORD` pra produto sem nenhum saldo no CISS. 397 testes no total, typecheck e build limpos. **Não mergeado. Não deployado. Migration não aplicada em produção.** Os 16 produtos PC mal-rotulados e o produto KG atuais na Wake **não foram corrigidos** (fora de escopo). |
