...
- **Tabela 74 — releitura pós-escrita não cresce mais com o número de lotes** (lacuna real: com P páginas na tabela e B lotes de escrita, o código antigo relia a tabela inteira DENTRO do loop de cada lote, gerando `P*B` GETs — suficiente pra estourar o rate limit da Wake, 120 req/min, quando muitos produtos mudam de uma vez): `syncPrices()` (`src/lib/sync/engine.ts`) agora escreve todos os lotes primeiro e só depois faz uma única releitura paginada cobrindo tudo — no máximo 2 leituras por run (1 diff inicial + 1 verificação final), nunca crescendo com o número de lotes. Teste dedicado prova a fórmula com 110 entradas na tabela e 110 alterações em 3 lotes (6 leituras, nunca 12).
...
- **Lock só pode ser liberado pelo dono** (fix já aplicado em fase anterior, confirmado correto e agora com suíte dedicada — antes não existia nenhum teste unitário de `lock.ts`): `acquireLock()` devolve um token único por aquisição (`lockedBy#pid<PID>`); `releaseLock()` só limpa o lock se o token bater com o dono atual. Sem isso, um TTL expirado (ou PID do dono morto) deixava outro processo roubar o lock via `UPDATE` condicional, mas o dono original, ao terminar, chamava `releaseLock(resource)` sem condição nenhuma — limpando o lock do novo dono legítimo enquanto ele ainda rodava. 11 testes novos (`src/lib/sync/lock.test.ts`): acquire/release básico, no-op de release com token errado, roubo por TTL expirado, roubo por PID morto, o cenário central da corrida (release do dono antigo com token velho vira no-op depois do lock passar a um novo dono), `withLocks()` (ordem alfabética, liberação em `finally`, falha parcial no meio da lista) e `recoverWorkerOrphans()`.
