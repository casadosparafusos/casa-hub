...
`WAKE_STOCK_CONTROL_MODE` é textual (não cai nas faixas numéricas de `SETTING_RANGES` acima) e antes aceitava qualquer string não-vazia — um typo no admin (ex.: `"fstroe"`) passava sem checagem de conjunto válido nenhuma. **Correção de precisão (revisão Tech Lead PR #4, revisão final #2, item 7): a versão anterior deste parágrafo afirmava que o motor de sync "tratava silenciosamente como se fosse `'erp'`" um valor inválido — isso é impreciso, não existe (nem existiu) nenhum ponto do motor de sync (`runSyncLocked()`/`syncPrices()`/`syncStock()`) que leia essa setting e ramifique comportamento por ela; a setting é hoje um gate/registro operacional, validado apenas na gravação (`PUT /api/settings`) e no pré-flight (`checkRequiredUnconfirmed()`), nunca consultado dentro do motor de sync em si.** Nova estrutura paralela `SETTING_ENUM_RANGES`/`validateEnumSettingValue()`/`isValidEnumSettingValue()` em `src/lib/settings.ts` — hoje só com `WAKE_STOCK_CONTROL_MODE: ['fstore', 'erp']` — valida exatamente esse conjunto (case-insensitive na entrada, sempre persistindo o valor canônico em minúsculo e trimado), tanto em `checkRequiredUnconfirmed()` (presente-mas-inválido bloqueia sync real igual a ausente) quanto no `PUT /api/settings` (400 antes de persistir, nunca grava o texto bruto digitado).

### `erp` é um valor válido, mas bloqueia escrita real de estoque (revisão Tech Lead PR #4, revisão final #2, item 1)

O fix acima garante que `WAKE_STOCK_CONTROL_MODE` só aceita `'fstore'`/`'erp'`, mas isso por si só não impedia um problema mais sério: `erp` é um modo *registrável*, porém o app nunca implementou o fluxo de baixa de estoque por pedido/evento do ERP que esse modo pressupõe — só o fluxo `fstore` (reconciliação por leitura de estoque físico) está implementado. Antes desta rodada, nada impedia alguém de configurar `WAKE_STOCK_CONTROL_MODE=erp` e disparar um sync real de estoque (`kind: 'stock'` ou `'both'`, `dryRun: false`) que seguiria pelo único caminho de escrita que existe (`fstore`), com a config dizendo outra coisa — uma discrepância silenciosa entre o que o operador configurou e o que o app de fato faz.

Correção: `runSyncLocked()` agora falha fechado *antes* de criar o `sync_run` e antes de qualquer escritor ser chamado, sempre que `kind` inclui estoque (`'stock'` ou `'both'`) **e** `dryRun === false` **e** `WAKE_STOCK_CONTROL_MODE === 'erp'`, lançando um erro explícito `WAKE_STOCK_CONTROL_MODE_UNSUPPORTED_FOR_REAL_STOCK_SYNC`. `dryRun: true` continua permitido em qualquer modo (preview nunca escreve). `kind: 'price'` nunca é afetado por esse guard — preço não depende do modo de controle de estoque. Nenhum fluxo de baixa por pedido foi implementado nesta rodada (fora de escopo pedido pela revisão); o guard só fecha a porta de um write real que não tem implementação por trás. Testes novos em `engine.test.ts` cobrem os 4 quadrantes (`stock`/`both` × `erp`/`fstore`, com e sem `dryRun`) mais o caso `kind: 'price'` com `erp` (não bloqueado).
